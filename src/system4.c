...
static void ain_scan_opcodes(FILE *f, struct ain *ain)  
{  
    bool seen_opcodes[NR_OPCODES] = { false };  
      
    for (size_t addr = 0; addr < ain->code_size;) {  
        uint16_t raw_opcode = LittleEndian_getW(ain->code, addr);  
        uint16_t opcode = raw_opcode & ~OPTYPE_MASK;  
          
        if (opcode >= NR_OPCODES) {  
            fprintf(f, "WARNING: Unknown opcode 0x%04X at address 0x%08zX\n",   
                    raw_opcode, addr);  
            break;  
        }  
...
static void usage(void)
{
	...
	puts("        --scan-opcodes   Scan and list all VM opcodes used in AIN file");
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		static struct option long_options[] = {
			...
			{ "scan-opcodes", no_argument, 0, LOPT_SCAN_OPCODES },
#endif
			{ 0 }
		};
		int option_index = 0;
