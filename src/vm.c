...
static enum opcode execute_instruction(enum opcode opcode)
{
	...
	case DG_STR_TO_METHOD: {  
	    ...
	    if (ain->version >= 11) {  
	        dg_type = get_argument(0);  
	    } else {  
	        dg_type = stack_pop().i;  
	    }  
	      
	    if (dg_type < 0 || dg_type >= ain->nr_delegates)  
	        VM_ERROR("Invalid delegate type: %d", dg_type);  
	      
	    int str_slot = stack_pop().i;  
	    int obj = stack_pop().i;  
	      
	    struct string *func_name = heap_get_string(str_slot);  
	      
	    // 使用 get_function_by_name 获取函数编号  
	    int func_no = get_function_by_name(func_name->text);  
	      
	    if (func_no < 0) {  
	        VM_ERROR("Function not found: %s", display_sjis0(func_name->text));  
	    }  
