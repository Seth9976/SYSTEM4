...
int controller_stack[32];  
int controller_stack_ptr = 0;
int PE_GetActiveController(void) {  
    return active_controller;  
}  
  
// 设置活动控制器  
void PE_SetActiveController(int controller_id) {  
    active_controller = controller_id;  
}  
  
// 压入控制器  
void PE_PushGUIController(int parts_no) {  
    if (controller_stack_ptr < 32) {  
        controller_stack[controller_stack_ptr++] = active_controller;  
        active_controller = parts_no;  
    }  
}  
  
// 弹出控制器  
void PE_PopGUIController(void) {  
    if (controller_stack_ptr > 0) {  
        active_controller = controller_stack[--controller_stack_ptr];  
    }  
}
